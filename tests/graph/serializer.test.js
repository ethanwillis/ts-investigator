...
const serializer_js_1 = require("../../src/graph/serializer.js");
...
function makeMinimalGraph(overrides = {}) {
    return {
...
describe('serializeGraph', () => {
    it('returns ok with valid JSON string for a minimal graph', () => {
        ...
        const result = (0, serializer_js_1.serializeGraph)(graph);
    ...
    it('serialized JSON contains expected top-level fields', () => {
        ...
        if (result.isOk()) {
            ...
            expect(parsed['version']).toBe(types_js_1.GRAPH_SCHEMA_VERSION);
    ...
    it('serializes ReadonlyMap nodes as a plain JSON object (record)', () => {
        ...
        const nodes = new Map([[funcNode.id, funcNode]]);
        const graph = makeMinimalGraph({ nodes: nodes });
        const result = (0, serializer_js_1.serializeGraph)(graph);
        expect(result.isOk()).toBe(true);
        if (result.isOk()) {
            const parsed = JSON.parse(result.value);
...
describe('deserializeGraph', () => {
    it('returns VALIDATION_FAILED for non-JSON input', () => {
        const result = (0, serializer_js_1.deserializeGraph)('not json at all {{{');
    ...
    it('returns VALIDATION_FAILED for valid JSON that fails schema', () => {
        ...
        const result = (0, serializer_js_1.deserializeGraph)(badJson);
        expect(result.isErr()).toBe(true);
        if (result.isErr()) {
    ...
    it('returns VALIDATION_FAILED for graph missing required fields', () => {
        ...
        const result = (0, serializer_js_1.deserializeGraph)(partial);
        expect(result.isErr()).toBe(true);
        if (result.isErr()) {
            expect(result.error.code).toBe('VALIDATION_FAILED');
        }
    ...
    it('reconstructs nodes as a ReadonlyMap (has Map methods)', () => {
        const graph = makeMinimalGraph();
        const serialized = (0, serializer_js_1.serializeGraph)(graph);
        expect(serialized.isOk()).toBe(true);
        if (serialized.isOk()) {
            const result = (0, serializer_js_1.deserializeGraph)(serialized.value);
            expect(result.isOk()).toBe(true);
            if (result.isOk()) {
    ...
    it('deserializes a VALIDATION_FAILED error with useful message', () => {
        const result = (0, serializer_js_1.deserializeGraph)('{"wrong": true}');
        expect(result.isErr()).toBe(true);
        if (result.isErr()) {
...
describe('serializeGraph / deserializeGraph round-trip', () => {
    ...
    it('round-trips a FunctionNode correctly', () => {
        ...
        const nodes = new Map([[funcNode.id, funcNode]]);
        const graph = makeMinimalGraph({ nodes: nodes });
        const rt = (0, serializer_js_1.deserializeGraph)((0, serializer_js_1.serializeGraph)(graph)._unsafeUnwrap())._unsafeUnwrap();
    ...
    it('round-trips a ClassNode correctly', () => {
        ...
        const nodes = new Map([[classNode.id, classNode]]);
        const graph = makeMinimalGraph({ nodes: nodes });
        const rt = (0, serializer_js_1.deserializeGraph)((0, serializer_js_1.serializeGraph)(graph)._unsafeUnwrap())._unsafeUnwrap();
    ...
    it('round-trips an InterfaceNode correctly', () => {
        ...
        const nodes = new Map([[ifaceNode.id, ifaceNode]]);
        const graph = makeMinimalGraph({ nodes: nodes });
        const rt = (0, serializer_js_1.deserializeGraph)((0, serializer_js_1.serializeGraph)(graph)._unsafeUnwrap())._unsafeUnwrap();
    ...
    it('round-trips a TypeAliasNode correctly', () => {
        ...
        const nodes = new Map([[typeAliasNode.id, typeAliasNode]]);
        const graph = makeMinimalGraph({ nodes: nodes });
        const rt = (0, serializer_js_1.deserializeGraph)((0, serializer_js_1.serializeGraph)(graph)._unsafeUnwrap())._unsafeUnwrap();
    ...
    it('round-trips a ModuleNode correctly', () => {
        ...
        const nodes = new Map([[moduleNode.id, moduleNode]]);
        const graph = makeMinimalGraph({ nodes: nodes });
        const rt = (0, serializer_js_1.deserializeGraph)((0, serializer_js_1.serializeGraph)(graph)._unsafeUnwrap())._unsafeUnwrap();
...
describe('writeGraph', () => {
    ...
    it('writes a valid graph file to disk', () => {
        ...
        const result = (0, serializer_js_1.writeGraph)(filePath, graph);
        expect(result.isOk()).toBe(true);
    ...
    it('written file contains valid JSON', () => {
        const graph = makeMinimalGraph();
        const filePath = path.join(tmpDir, 'graph.json');
        (0, serializer_js_1.writeGraph)(filePath, graph);
...
describe('readGraph', () => {
    ...
    it('returns IO_ERROR when file does not exist', () => {
        ...
        const result = (0, serializer_js_1.readGraph)(filePath);
        expect(result.isErr()).toBe(true);
        if (result.isErr()) {
            expect(result.error.kind).toBe('SerializerError');
            expect(result.error.code).toBe('IO_ERROR');
    ...
    it('reads back a graph written by writeGraph', () => {
        ...
        const graph = makeMinimalGraph({ nodes: nodes });
        const filePath = path.join(tmpDir, 'graph.json');
        const writeResult = (0, serializer_js_1.writeGraph)(filePath, graph);
        expect(writeResult.isOk()).toBe(true);
        const readResult = (0, serializer_js_1.readGraph)(filePath);
    ...
    it('writeGraph then readGraph preserves all edge kinds', () => {
        ...
        if (result.isOk()) {
            ...
            expect(edges.map((e) => e.kind).sort()).toEqual(['call', 'import', 'inherits', 'references'].sort());
        }
    });
});
