...
const formGenerator_js_1 = require("../../src/server/formGenerator.js");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeStringTypeInfo() {
    return { kind: 'primitive', name: 'string' };
}
function makeNumberTypeInfo() {
    return { kind: 'primitive', name: 'number' };
}
function makeBooleanTypeInfo() {
...
describe('FieldFactory.typeInfoToField', () => {
    let factory;
    beforeEach(() => {
        factory = new formGenerator_js_1.FieldFactory();
    });
    // ---- Primitive: string ---------------------------------------------------
    ...
    describe('remaining primitive kinds', () => {
        ...
        primitiveTextCases.forEach(({ name }) => {
            it(`primitive "${name}" → { kind: "text" } with label hint`, () => {
                const typeInfo = { kind: 'primitive', name: name };
        ...
        it('primitive "unknown" → { kind: "textarea" }', () => {
            const typeInfo = { kind: 'primitive', name: 'unknown' };
            const field = factory.typeInfoToField('p', 'P', typeInfo, false);
            expect(field.kind).toBe('textarea');
        });
        it('primitive "any" → { kind: "textarea" }', () => {
            const typeInfo = { kind: 'primitive', name: 'any' };
            const field = factory.typeInfoToField('p', 'P', typeInfo, false);
            expect(field.kind).toBe('textarea');
        });
    });
    ...
    describe('literal type', () => {
        it('maps a string literal → { kind: "select", options: [value] }', () => {
            const typeInfo = { kind: 'literal', value: 'admin' };
        ...
        it('maps a number literal → { kind: "select", options: ["42"] }', () => {
            const typeInfo = { kind: 'literal', value: 42 };
        ...
        it('maps a boolean literal → { kind: "select", options: ["true"] }', () => {
            const typeInfo = { kind: 'literal', value: true };
    ...
    describe('unknown type → textarea', () => {
        it('maps unknown type → { kind: "textarea" }', () => {
            const typeInfo = { kind: 'unknown', raw: 'ComplexType<X>' };
            const field = factory.typeInfoToField('x', 'X', typeInfo, false);
            expect(field.kind).toBe('textarea');
        });
...
describe('renderFormHtml', () => {
    ...
    it('returns a string', () => {
        const html = (0, formGenerator_js_1.renderFormHtml)(makeForm());
    ...
    it('contains the function name in the output', () => {
        const html = (0, formGenerator_js_1.renderFormHtml)(makeForm({ functionName: 'greet' }));
        expect(html).toContain('greet');
    });
    it('contains the function id somewhere in the form id attribute', () => {
        const html = (0, formGenerator_js_1.renderFormHtml)(makeForm({ functionId: 'src/utils.ts#function:greet' }));
    ...
    it('contains a "no parameters" message for a zero-field form', () => {
        const html = (0, formGenerator_js_1.renderFormHtml)(makeForm({ fields: [] }));
    ...
    it('renders a text input for a text field', () => {
        ...
        const html = (0, formGenerator_js_1.renderFormHtml)(form);
    ...
    it('renders all fields from a parametersToForm result', () => {
        ...
        const factory = new formGenerator_js_1.FieldFactory();
        const form = factory.parametersToForm(node);
        const html = (0, formGenerator_js_1.renderFormHtml)(form);
        expect(html).toContain('<form');
...
//# sourceMappingURL=formGenerator.test.js.map