...
const graphRenderer_js_1 = require("../../src/server/graphRenderer.js");
const types_js_1 = require("../../src/graph/types.js");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeEmptyGraph(overrides = {}) {
    return {
...
function makeInterfaceNode(overrides = {}) {
    return {
        kind: 'interface',
...
function makeTypeAliasNode(overrides = {}) {
    return {
        kind: 'typeAlias',
...
function makeGraphWithNodes(nodes) {
    const nodeMap = new Map(nodes.map((n) => [n.id, n]));
    return makeEmptyGraph({ nodes: nodeMap });
}
function makeGraphWithEdges(nodes, edges) {
    const nodeMap = new Map(nodes.map((n) => [n.id, n]));
...
describe('buildDotString', () => {
    describe('output structure', () => {
        it('returns a string', () => {
            const dot = (0, graphRenderer_js_1.buildDotString)(makeEmptyGraph());
        ...
        it('contains rankdir=LR for left-to-right layout', () => {
            const dot = (0, graphRenderer_js_1.buildDotString)(makeEmptyGraph());
    ...
    describe('FunctionNode in DOT output', () => {
        it('contains the node id quoted in the DOT output', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithNodes([node]));
    ...
    describe('multiple nodes', () => {
        it('contains all node ids when multiple nodes are present', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithNodes([funcNode, classNode, moduleNode]));
        ...
        it('contains all node names when multiple nodes are present', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithNodes([funcNode, classNode]));
            expect(dot).toContain('greet');
            expect(dot).toContain('App');
        });
    ...
    describe('CallEdge in DOT output', () => {
        it('contains "->" for a CallEdge', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([from, to], [edge]));
            expect(dot).toContain('->');
        });
        ...
        it('call edge uses style="solid"', () => {
            const from = makeFunctionNode({ id: 'src/index.ts#function:run', name: 'run' });
            const to = makeFunctionNode({ id: 'src/utils.ts#function:greet', name: 'greet' });
            const edge = { kind: 'call', from: from.id, to: to.id, line: 5 };
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([from, to], [edge]));
        ...
        it('call edge uses call edge color "#4A90D9"', () => {
            ...
            const edge = { kind: 'call', from: from.id, to: to.id, line: 1 };
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([from, to], [edge]));
    ...
    describe('InheritsEdge in DOT output', () => {
        it('contains "->" for an InheritsEdge', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([child, parent], [edge]));
            expect(dot).toContain('->');
        });
        it('inherits edge uses style="bold"', () => {
            const child = makeClassNode({ id: 'src/foo.ts#class:Child', name: 'Child' });
            const parent = makeClassNode({ id: 'src/foo.ts#class:Parent', name: 'Parent' });
            const edge = { kind: 'inherits', from: child.id, to: parent.id };
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([child, parent], [edge]));
    ...
    describe('ReferencesEdge in DOT output', () => {
        it('contains "->" for a ReferencesEdge', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([func, iface], [edge]));
            expect(dot).toContain('->');
        });
        it('references edge uses style="dotted"', () => {
            const func = makeFunctionNode();
            const iface = makeInterfaceNode();
            const edge = { kind: 'references', from: func.id, to: iface.id };
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([func, iface], [edge]));
    ...
    describe('edge style by kind', () => {
        ...
        edgeStyleCases.forEach(({ label, edge, expectedStyle }) => {
            it(`${label}`, () => {
                ...
                const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([fromNode, toNode], [edge]));
    ...
    describe('multiple edges', () => {
        it('produces "->" for each edge in the graph', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([a, b, c], edges));
        ...
        it('all four edge kinds can coexist in one graph', () => {
            ...
            const dot = (0, graphRenderer_js_1.buildDotString)(makeGraphWithEdges([modA, modB, funcA, funcB, classChild, classParent, iface], edges));
