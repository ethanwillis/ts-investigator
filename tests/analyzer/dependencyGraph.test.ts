...
import { buildGraph } from '../../src/analyzer/dependencyGraph.js';
import type {
  DependencyGraph,
  FunctionNode,
  ClassNode,
...
beforeAll(() => {
  ...
  const graphResult = buildGraph(scanResult.value);
...
describe('buildGraph', () => {
  describe('overall result', () => {
    it('returns a DependencyGraph (not an error) for the fixture project', () => {
      ...
      if (scanResult.isErr()) return;

      const result = buildGraph(scanResult.value);
  ...
  describe('FunctionNode for "greet"', () => {
    let greetNode: FunctionNode | undefined;
  ...
  describe('ClassNode for "App"', () => {
    let appNode: ClassNode | undefined;
    ...
    it('has a method named "App.run" or "run"', () => {
      ...
      const runMethod =
        appNode?.methods.find((m) => m.name === 'App.run') ??
    ...
    it('"admin", "user", "guest" are among the union literal values', () => {
      ...
      if (node?.kind === 'typeAlias' && node.typeInfo.kind === 'union') {
        const values = node.typeInfo.members
          .filter((m): m is Extract<typeof m, { kind: 'literal' }> => m.kind === 'literal')
          .map((m) => m.value);
  ...
  describe('buildGraph options', () => {
    it('accepts a custom maxTypeDepth option without error', () => {
      ...
      const result = buildGraph(scanResult.value, { maxTypeDepth: 3 });
      expect(result.isOk()).toBe(true);
    });
    ...
    it('skipDeclarationFiles: false still produces a valid graph', () => {
      ...
      const result = buildGraph(scanResult.value, { skipDeclarationFiles: false });
