...
describe('ProjectScanner.scan', () => {
    let scanner;
    beforeEach(() => {
        scanner = new projectScanner_js_1.ProjectScanner();
    });
    ...
    describe('when the project has no .ts source files', () => {
        ...
        beforeEach(() => {
            ...
            fs.writeFileSync(path.join(emptyProjectDir, 'tsconfig.json'), JSON.stringify(tsconfig, null, 2), 'utf-8');
            ...
            fs.writeFileSync(path.join(emptyProjectDir, 'src', 'stub.d.ts'), '// declaration stub — intentionally empty\nexport {};\n');
        });
        afterEach(() => {
