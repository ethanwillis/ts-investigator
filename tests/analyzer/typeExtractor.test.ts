...
import { TypeExtractor } from '../../src/analyzer/typeExtractor.js';
import type { TypeInfo } from '../../src/graph/types.js';

// ---------------------------------------------------------------------------
// Helpers — build a minimal in-memory TypeScript program
// ---------------------------------------------------------------------------

const IN_MEMORY_COMPILER_OPTIONS: ts.CompilerOptions = {
...
function createInMemoryProgram(files: Record<string, string>): {
  program: ts.Program;
  checker: ts.TypeChecker;
} {
  const defaultHost = ts.createCompilerHost(IN_MEMORY_COMPILER_OPTIONS);
 ...
 */
function getTypeFromAnnotation(
  annotation: string,
  preamble = '',
 ...
 */
function getSignatureFromFunction(funcCode: string): {
  ...
  let foundSig: ts.Signature | undefined;

  ts.forEachChild(sourceFile, (node) => {
    if (foundSig !== undefined) return;
...
function getPropertiesFromType(
  annotation: string,
  preamble = '',
): { properties: ts.Symbol[]; checker: ts.TypeChecker } {
...
describe('TypeExtractor.extractTypeInfo', () => {
  ...
  describe('primitive types', () => {
    const primitives: Array<{
      annotation: string;
      name: Extract<TypeInfo, { kind: 'primitive' }>['name'];
    }> = [
    ...
    it('maps "string | number" → { kind: "union", members: [string, number] }', () => {
      ...
      if (result.kind === 'union') {
        ...
        const names = result.members
          .filter((m): m is Extract<TypeInfo, { kind: 'primitive' }> => m.kind === 'primitive')
          .map((m) => m.name);
    ...
    it("maps \"'admin' | 'user' | 'guest'\" → union of 3 literals", () => {
      ...
      if (result.kind === 'union') {
        ...
        const values = result.members
          .filter((m): m is Extract<TypeInfo, { kind: 'literal' }> => m.kind === 'literal')
          .map((m) => m.value);
...
describe('TypeExtractor.extractParameters', () => {
  ...
  it('extracts a single string parameter', () => {
    const { signature, checker } = getSignatureFromFunction(
  ...
  it('extracts number and boolean parameters', () => {
    const { signature, checker } = getSignatureFromFunction(
      'function isAdult(age: number, strict: boolean): boolean { return strict ? age >= 18 : age > 16; }',
    );
    const params = extractor.extractParameters(signature, checker);
  ...
  it('sets isOptional: true for parameters with default values', () => {
    const { signature, checker } = getSignatureFromFunction(
      'function withDefault(count: number = 10): void {}',
    );
    const params = extractor.extractParameters(signature, checker);
  ...
  it('sets isRest: true for rest parameters', () => {
    const { signature, checker } = getSignatureFromFunction(
      'function formatList(sep: string, ...items: string[]): string { return items.join(sep); }',
    );
    const params = extractor.extractParameters(signature, checker);
  ...
  it('does not set defaultValue on a parameter without a default', () => {
    const { signature, checker } = getSignatureFromFunction(
      'function noDefault(x: number): void {}',
    );
    const params = extractor.extractParameters(signature, checker);
  ...
  it('extracts a union type parameter correctly', () => {
    const { signature, checker } = getSignatureFromFunction(
      "function assignRole(role: 'admin' | 'user' | 'guest'): void {}",
    );
    const params = extractor.extractParameters(signature, checker);
